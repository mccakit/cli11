...
export namespace CLI
{
    ...
    // =============================================
    // Implementation (from Config_inl.hpp)
    // =============================================

    constexpr auto multiline_literal_quote = R"(''')";
    constexpr auto multiline_string_quote = R"(""")";
    ...
    std::string ConfigBase::to_config(const App *app,
                                      bool default_also,
            ...
            for (const Option *opt : app->get_options({}))
            {
                ...
                if (opt->get_configurable())
                {
                    ...
                    if (opt->get_multi_option_policy() == CLI::MultiOptionPolicy::Sum && opt->count() >= 1 &&
                        results.size() == 1)
                    {
                        // if the multi option policy is sum then there is a possibility of incorrect fields being
                        // produced best to just use the original data for config files
                    ...
                    if (opt->get_multi_option_policy() == CLI::MultiOptionPolicy::Join && opt->count() > 1)
                    {
                        ...
                        if (delim == '\0')
                        {
                            // this branch deals with a situation where the output would not be readable by a config
                            // file
                            results = opt->results();
                        }
                        else
